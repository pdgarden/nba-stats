...
team as (
    ...
    select distinct home_team id  from game_summary
...
game_summary_stack as (
    select * from game_summary_home
    union all
    select * from game_summary_away
...
final as (
    ...
    select
        ...
        sum(
            (
                (gss.is_home_team and gss.home_team_won)
                or
                (not gss.is_home_team and not gss.home_team_won)
            )::int) nb_game_win,
        avg(
            (
                (gss.is_home_team and gss.home_team_won)
